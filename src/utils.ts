import type { Category, MenuItem, RestaurantLocation } from "./types";
...
export const getUserLocation = async (
  ...
  const order_location = await getAllLocations(origin).then((locations) =>
    locations.find(({ id }) => id === restaurant_id)
  );
...
export const getAllLocations = async (
  ...
  return (await fetch(new URL("/api/restaurant-locations", origin).href).then(
    (response) => response.json()
