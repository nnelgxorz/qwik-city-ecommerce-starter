...
import { type EndpointHandler, useEndpoint } from "@builder.io/qwik-city";
...
export const onGet: EndpointHandler<PageContent> = async (event) => {
  ...
  const items = await getRestaurantMenu(origin).then((restaurant_menu) =>
    restaurant_menu.filter(({ categories }) => {
