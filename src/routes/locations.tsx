...
export const Location = component$((props: { location: RestaurantLocation }) => {
  return <li>
    <article>
      <h3>{props.location.name}</h3>
      <p>{props.location.address.street} {props.location.address.city}</p>
      <form action="/api/select-location" method="post">
        <input type="text" name="location" hidden aria-hidden="true" readOnly value={props.location.id} />
        <button aria-label={`Begin online order for ${props.location.name}`}>
          Order Online
        </button>
      </form>
    </article>
...
export default component$(() => {
  ...
  return <Host>
    ...
    <Async resource={resource}
      ...
      onResolved={(locations) => (<ul class="grid gap-1" role="list">
